...
def zzt_img(*args, **kwargs):
    # Set up zzt/szzt specifics
    CHAR_WIDTH = 16 if kwargs.get("szzt") else 8
    CHAR_HEIGHT = 16 if kwargs.get("szzt") else 14
    ENGINE = "szzt" if kwargs.get("szzt") else "zzt"
    ...
    if has_coords:
        ...
        left = tl[0] * CHAR_WIDTH
        top = tl[1] * CHAR_HEIGHT
        width = (br[0] - tl[0] + 1) * CHAR_WIDTH
        height = (br[1] - tl[1] + 1) * CHAR_HEIGHT
    ...
    div = "div class='engine-{} zzt-img{}' style='{}'".format(ENGINE, shorthand, css)
    ...
    if message_crop:
        ...
        output += "<div class='zzt-txt-message'><span class='green'>{}</span></div>\n".format(message)
